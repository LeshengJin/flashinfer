...
#include <hip/hip_fp8.h> // TODO: this isn't the right header
#endif
...
template <size_t vec_size>
FLASHINFER_INLINE void vec_cast(float* dst, const half* src) {
#pragma unroll
...
template <size_t vec_size>
FLASHINFER_INLINE void vec_cast(half* dst, const float* src) {
#pragma unroll
  for (size_t i = 0; i < vec_size / 2; ++i) {
...
#ifdef FLASHINFER_ENABLE_BF16
template <size_t vec_size>
FLASHINFER_INLINE void vec_cast(float* dst, const nv_bfloat16* src) {
#pragma unroll
  for (size_t i = 0; i < vec_size / 2; ++i) {
...
template <size_t vec_size>
FLASHINFER_INLINE void vec_cast(nv_bfloat16* dst, const float* src) {
#pragma unroll
  for (size_t i = 0; i < vec_size / 2; ++i) {
