...
constexpr BlockScanAlgorithm SCAN_ALGO = BLOCK_SCAN_RAKING; //BLOCK_SCAN_WARP_SCANS;
constexpr BlockReduceAlgorithm REDUCE_ALGO = BLOCK_REDUCE_WARP_REDUCTIONS; //BLOCK_REDUCE_WARP_REDUCTIONS;
...
#pragma unroll
    ...
    BlockAdjacentDifference<bool, BLOCK_THREADS>(temp_storage->block_prim.adj_diff)
        .SubtractLeft(greater_than_u, greater_than_u, BoolDiffOp());
    __syncthreads();
...
hipError_t SamplingFromProb(T* probs, T* uniform_samples, IdType* output, uint32_t batch_size,
                             uint32_t d, hipStream_t stream = 0) {
...
template <typename T, typename IdType>
hipError_t ParallelSamplingFromProb(T* probs, T* uniform_samples, IdType* output,
...
template <typename T, typename IdType>
hipError_t TopKSamplingFromProb(T* probs, T* uniform_samples, IdType* output, bool* success,
                                 IdType top_k, uint32_t batch_size, uint32_t d,
                                 uint32_t max_top_k_rounds, hipStream_t stream = 0) {
  constexpr uint32_t BLOCK_THREADS = 1024;
  const uint32_t vec_size = std::gcd(16 / sizeof(T), d);
...
template <typename T, typename IdType>
hipError_t TopPSamplingFromProb(T* probs, T* uniform_samples, IdType* output, bool* success,
                                 T top_p, uint32_t batch_size, uint32_t d,
                                 uint32_t max_top_p_rounds, hipStream_t stream = 0) {
  constexpr uint32_t BLOCK_THREADS = 1024;
  const uint32_t vec_size = std::gcd(16 / sizeof(T), d);
...
template <typename T, typename IdType>
hipError_t ParallelTopPSamplingFromProb(T* probs, T* uniform_samples, IdType* output,
